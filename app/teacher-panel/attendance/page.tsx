...
const statusConfig: Record<
	AttendanceStatus,
	{ label: string; icon: React.ElementType; cls: string; activeCls: string }
> = {
	present: {
		label: 'Keldi',
		icon: CheckCircle2,
		cls: 'border-slate-700 text-slate-400 hover:border-emerald-500/50',
		activeCls: 'bg-emerald-500/15 border-emerald-500 text-emerald-400',
	},
	late: {
		label: 'Kech',
		icon: Clock,
		cls: 'border-slate-700 text-slate-400 hover:border-amber-500/50',
		activeCls: 'bg-amber-500/15 border-amber-500 text-amber-400',
	},
	absent: {
		label: 'Kelmadi',
		icon: AlertCircle,
		cls: 'border-slate-700 text-slate-400 hover:border-red-500/50',
		activeCls: 'bg-red-500/15 border-red-500 text-red-400',
	},
}
		...
		<div className='min-h-screen bg-slate-950 text-white p-6 space-y-6'>
			...
			<div className='flex flex-wrap gap-3 items-end'>
				<div className='flex-1 min-w-[200px]'>
					...
					<select
						...
						className='w-full h-10 px-3 text-sm bg-slate-900 border border-white/10 rounded-xl text-white focus:outline-none focus:border-blue-500 transition-colors'
					>
				...
				<div>
					...
					<input
						...
						className='h-10 px-3 text-sm bg-slate-900 border border-white/10 rounded-xl text-white focus:outline-none focus:border-blue-500 transition-colors'
					/>
				</div>
			</div>
			...
			{selectedGroup && !loading && students.length > 0 && (
				<div className='flex gap-3 flex-wrap'>
					{[
						{ status: 'present' as AttendanceStatus, count: counts.present },
						{ status: 'late' as AttendanceStatus, count: counts.late },
						{ status: 'absent' as AttendanceStatus, count: counts.absent },
					].map(({ status, count }) => {
			...
			{!selectedGroup ? (
				<div className='text-center py-20 bg-slate-900/50 border border-white/5 rounded-2xl'>
					...
					<p className='text-slate-500'>Guruhni tanlang</p>
				</div>
			) : loading ? (
				<div className='flex items-center justify-center py-20 gap-3 bg-slate-900/50 border border-white/5 rounded-2xl'>
				...
				<div className='text-center py-20 bg-slate-900/50 border border-white/5 rounded-2xl'>
					<Users className='w-12 h-12 text-slate-700 mx-auto mb-3' />
					<p className='text-slate-500'>Bu guruhda talaba yo'q</p>
				</div>
			) : (
				<>
					<div className='bg-slate-900/80 backdrop-blur-sm border border-white/5 rounded-2xl overflow-hidden'>
						<table className='w-full text-sm'>
							<thead>
								<tr className='border-b border-white/5'>
									<th className='text-left text-[10px] font-black text-slate-500 uppercase tracking-widest px-5 py-3'>
										#
									</th>
									<th className='text-left text-[10px] font-black text-slate-500 uppercase tracking-widest px-5 py-3'>
										Talaba
					...
					<div className='flex justify-end'>
						<button
							onClick={handleSave}
							disabled={saving}
							className='flex items-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 text-white font-black text-sm h-11 px-8 rounded-xl transition-all hover:scale-105 shadow-lg shadow-blue-500/20'
						>
							{saving ? (
								<>
