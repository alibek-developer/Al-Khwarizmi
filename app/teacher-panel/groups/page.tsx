			...
			{!selectedGroup ? (
				...
				groups.length === 0 ? (
					<div className='text-center py-20 bg-slate-900/50 border border-white/5 rounded-2xl'>
						...
						<p className='text-slate-500'>Guruhlar topilmadi</p>
					</div>
				) : (
					<div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-4'>
						{groups.map((g, idx) => (
							<button
								key={g.id}
								onClick={() => openGroup(g)}
								className='text-left bg-slate-900/80 backdrop-blur-sm border border-white/5 rounded-2xl p-5 hover:border-blue-500/30 hover:-translate-y-1 transition-all duration-300 group'
							>
								<div
								...
								<h3 className='font-black text-white text-lg mb-1 group-hover:text-blue-400 transition-colors'>
									{g.name}
								</h3>
								{g.schedule && (
				...
				<>
					{/* Group info card */}
					<div className='bg-slate-900/80 border border-white/5 rounded-2xl p-4 flex items-center gap-4'>
						<div
						...
						<div>
							<h2 className='font-black text-white'>{selectedGroup.name}</h2>
						...
						<div className='ml-auto text-right'>
							...
							<div className='text-[11px] text-slate-500'>talaba</div>
						</div>
					</div>
					...
					<div className='relative max-w-xs'>
						...
						<input
							...
							className='w-full h-9 pl-9 pr-4 text-xs bg-slate-900 border border-white/10 rounded-xl text-slate-300 placeholder-slate-600 focus:outline-none focus:border-blue-500 transition-colors'
						/>
					</div>
					...
					{studentsLoading ? (
						...
						<div className='text-center py-16 bg-slate-900/50 border border-white/5 rounded-2xl'>
							...
							<p className='text-slate-500 text-sm'>Talaba topilmadi</p>
						</div>
					) : (
