  ...
  useEffect(() => {
    ...
    const fetchStudent = async () => {
      try {
        ...
        setStudent({
          id: 0,
          full_name: name || t('breadcrumb_student'),
          email: email || "",
        });
      } finally {
        setLoading(false);
      ...
      >
        ...
        <div className="px-2 pb-3 pt-2 border-t border-slate-100 dark:border-slate-800 space-y-0.5 shrink-0">
          ...
          >
            ...
            />
            {!collapsed && <span>{locale === "en" ? 'English' : "O'zbekcha"}</span>}
          </button>
          ...
          >
            ...
            {!collapsed && <span>{dark ? t('light_mode') : t('dark_mode')}</span>}
          </button>

          <button
            onClick={handleLogout}
            title={collapsed ? t('layout_logout') : undefined}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 text-sm">
              <span className="text-slate-400 dark:text-slate-500 font-medium">
                {t('breadcrumb_student')}
              </span>
              <span className="text-slate-300 dark:text-slate-700">/</span>
              <span className="font-bold text-slate-900 dark:text-white">
                {t('layout_' + (currentPage?.key ?? 'dashboard'))}
              </span>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2.5 pl-2 border-l border-slate-200 dark:border-slate-800 ml-1">
              ...
              <div className="hidden sm:block leading-none">
                <p className="text-xs font-bold text-slate-900 dark:text-white">
                  {student?.full_name || t('breadcrumb_student')}
                </p>
                <p className="text-[10px] text-slate-400 truncate max-w-[140px]">
                  {student?.email || t('layout_title')}
                </p>
              </div>
            </div>
          </div>
