...
const mockGames = [
	{
		...
		xp: 50,
		unlocked: true,
		route: '/student/dashboard/games/logic-puzzles',
	},
	{
		id: 2,
		name: 'Quick Math',
		...
		xp: 40,
		unlocked: true,
		route: '/student/dashboard/games/quick-math',
	},
	{
		id: 3,
		name: 'Word Match',
		...
		xp: 35,
		unlocked: true,
		route: '/student/dashboard/games/word-match',
	},
	{
		id: 4,
		name: 'Memory Game',
		...
		route: '/student/dashboard/games/memory-game',
	},
	{
		id: 5,
		name: 'Code Challenge',
...
export default function StudentGamesPage() {
	...
	const [gameStats, setGameStats] = useState<
		Record<string, { count: number; totalPoints: number }>
	>({})
	...
	const questions = [
		{
			question:
				"JavaScript-da 'let' va 'const' o'zgaruvchilari qachon e'lon qilinadi?",
			options: [
				'Funksiya ichida',
				'Blok ichida',
				'Global miqyosda',
	...
	const handleGameClick = (game: (typeof mockGames)[0]) => {
		if (!game.unlocked || !game.route) return
							...
							{mockGames.map(game => {
								...
								return (
									<div
										...
										className={`relative rounded-xl p-4 border transition-all group ${
											game.unlocked
												? `${colors.border} hover:shadow-md cursor-pointer hover:scale-[1.01]`
									...
									>
										...
										<div className='flex items-center gap-3'>
											<div
												className={`w-10 h-10 ${game.lightBg} rounded-xl flex items-center justify-center shrink-0`}
											>
											...
											{game.unlocked && (
												<ChevronRight
													className={`w-4 h-4 text-slate-300 dark:text-slate-600 shrink-0 transition-transform group-hover:translate-x-0.5 ${colors.text} opacity-0 group-hover:opacity-100`}
												/>
											)}
										</div>
							...
							<div>
								...
								<div className='h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden'>
									<div
										...
										style={{
											width: `${Math.min((totalXP / 2500) * 100, 100)}%`,
										}}
									/>
								</div>
							</div>
							<div>
								...
								<div className='h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden'>
									<div
										...
										style={{
											width: `${Math.min((totalGamesPlayed / 7) * 100, 100)}%`,
										}}
									/>
								</div>
							</div>
							<div>
								...
								<div className='h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden'>
									<div
										...
										style={{
											width: `${Math.min((Object.keys(gameStats).length / 5) * 100, 100)}%`,
										}}
									/>
								</div>
							</div>
						...
						<div className='grid grid-cols-2 gap-3'>
							...
							<div className='bg-slate-50 dark:bg-slate-800 rounded-xl p-3 text-center'>
								<p className='text-xl font-black text-blue-600 dark:text-blue-400'>
									{totalGamesPlayed > 0
										? Math.round(totalXP / totalGamesPlayed)
										: 0}
									%
								</p>
						  ...
						  selectedAnswer !== null ? (
							<div className='p-8 text-center'>
								...
								<p className='text-sm text-slate-500 dark:text-slate-400 mb-4'>
									Siz {score}/{questions.length} savolga to&apos;g&apos;ri javob
									berdingiz
								</p>
								...
								<div className='flex items-center justify-center gap-2 text-amber-600 dark:text-amber-400 mb-6'>
									...
									<span className='font-bold'>
										+{Math.round(mockDailyQuiz.xpReward * (finalScore / 100))}{' '}
										XP
									</span>
								</div>
								<button
