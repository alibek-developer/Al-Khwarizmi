...
import { dashboardApi, courseApi, gameApi } from '@/lib/api'
import {
	Award,
	BookOpen,
	CheckCircle2,
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const [statsData, progressData, leaderboardData, coursesData, gameStats] = await Promise.all([
					dashboardApi.getStats(id),
					dashboardApi.getCourseProgress(id),
					dashboardApi.getLeaderboard(5),
					courseApi.getByStudentId(id),
					gameApi.getStats(id),
				])
				setStats({
					...statsData,
					gamePoints: Object.values(gameStats as Record<string, { count: number; totalPoints: number }>)
			...
			<div className='grid grid-cols-2 lg:grid-cols-4 gap-4'>
				<div className='bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm hover:shadow-md transition-all'>
					<div className='flex items-start justify-between mb-3'>
						...
						<span className='flex items-center gap-0.5 text-[11px] font-bold px-1.5 py-0.5 rounded-lg bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400'>
							<TrendingUp className='w-3 h-3' /> +{stats.gamePoints}
						</span>
					</div>
			...
			<div className='grid lg:grid-cols-5 gap-4'>
					...
					<div className='flex items-center justify-between mb-5'>
						<div>
							<h3 className='font-black text-slate-900 dark:text-white text-sm'>
								{t('course_progress_heading')}
							</h3>
							<p className='text-[10px] text-slate-400 mt-0.5'>
								{t('course_progress_sub')}
							</p>
						</div>
				...
				<div className='lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm'>
					<div className='flex items-center gap-2 mb-4'>
						...
						<div>
							<h3 className='font-black text-slate-900 dark:text-white text-sm'>
								{t('leaderboard_heading')}
							</h3>
							<p className='text-[10px] text-slate-400'>{t('leaderboard_sub')}</p>
						</div>
					</div>
			...
			<div className='grid sm:grid-cols-2 lg:grid-cols-4 gap-4'>
				<div className='bg-gradient-to-br from-violet-500 to-purple-600 rounded-2xl p-5 text-white shadow-lg shadow-violet-500/20'>
					<div className='flex items-center gap-2 mb-3'>
						...
						<span className='text-xs font-semibold text-white/80'>{t('stat_course')}</span>
					</div>
					<h4 className='text-lg font-black mb-1 leading-tight'>
						{enrolledCourses[0]?.groups?.course?.title_uz ||
						 enrolledCourses[0]?.groups?.course?.title_en ||
						 courseProgress[0]?.group?.course?.title_uz ||
						 '—'}
					</h4>
				</div>
				...
				<div className='bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl p-5 text-white shadow-lg shadow-emerald-500/20'>
					<div className='flex items-center gap-2 mb-3'>
						...
						<span className='text-xs font-semibold text-white/80'>{t('stat_game_points')}</span>
					</div>
					<p className='text-3xl font-black'>{stats.gamePoints}</p>
				</div>
