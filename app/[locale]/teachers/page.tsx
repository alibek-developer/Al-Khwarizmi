	...
	const stats = [
		{
			...
			label: t('modal_stat_students'),
			color: 'text-blue-600',
		},
		{
			icon: Clock,
			value: mentor.experience || '—',
			label: t('modal_stat_experience'),
			color: 'text-emerald-600',
		},
		{
			icon: Star,
			value: mentor.rating?.toString() || '5.0',
			label: t('modal_stat_rating'),
					...
					{(mentor.about_en || mentor.about_uz) && (
						<div className='mb-5'>
							<h4 className='text-xs font-black text-slate-900 dark:text-white uppercase tracking-widest mb-2.5 flex items-center gap-2'>
								<span className='w-3 h-0.5 bg-blue-600 rounded-full' /> {t('modal_about')}
							</h4>
							{mentor.about_en && (
					...
					{mentor.skills?.length > 0 && (
						<div className='mb-5'>
							<h4 className='text-xs font-black text-slate-900 dark:text-white uppercase tracking-widest mb-3 flex items-center gap-2'>
								<span className='w-3 h-0.5 bg-blue-600 rounded-full' /> {t('modal_skills')}
							</h4>
				...
				<section className='py-16 bg-white dark:bg-slate-950'>
					<div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
						...
						<div className='grid md:grid-cols-3 gap-6'>
							{[
								{ metric: '1:1', key: 'philosophy_1' },
								{ metric: '100%', key: 'philosophy_2' },
								{ metric: '24/7', key: 'philosophy_3' },
							].map((item, idx) => (
								<div
									key={idx}
