...
function StatusBadge({ status, t }: { status: string; t: (key: string) => string }) {
	...
	return (
		<span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-bold ${s.cls}`}>
			<Icon className='w-2.5 h-2.5' /> {t(s.labelKey)}
		</span>
	)
}
...
export default function AdminDashboardPage() {
	...
	useEffect(() => {
		...
		else if (h < 17) setGreeting(t('greeting_afternoon'))
	...
	const statCards = [
		{
			id: 'students', label: t('stat_total_students'), value: String(studentCount), change: '', up: true,
			desc: t('stat_total_students'), icon: Users, light: 'bg-blue-50 dark:bg-blue-500/10',
			text: 'text-blue-600 dark:text-blue-400', border: 'border-blue-100 dark:border-blue-500/20',
		},
		{
			id: 'revenue', label: t('stat_total_revenue'), value: `$${totalRevenue.toLocaleString()}`, change: '', up: true,
			desc: t('desc_paid_payments'), icon: CreditCard, light: 'bg-emerald-50 dark:bg-emerald-500/10',
			text: 'text-emerald-600 dark:text-emerald-400', border: 'border-emerald-100 dark:border-emerald-500/20',
		},
		{
			id: 'courses', label: t('stat_active_courses'), value: String(courseCount), change: '', up: true,
			desc: t('desc_all_courses'), icon: BookOpen, light: 'bg-violet-50 dark:bg-violet-500/10',
			text: 'text-violet-600 dark:text-violet-400', border: 'border-violet-100 dark:border-violet-500/20',
		},
		{
			id: 'rating', label: t('stat_avg_rating'), value: String(avgRating), change: '', up: true,
			desc: t('desc_all_courses_avg'), icon: Star, light: 'bg-amber-50 dark:bg-amber-500/10',
		...
		<div className='space-y-5 pb-6'>
			<div className='flex items-center justify-between'>
				...
				<div className='hidden sm:flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2 shadow-sm'>
					...
					<span className='text-xs font-semibold text-slate-600 dark:text-slate-300'>{t('system_active')}</span>
				</div>
			</div>
			...
			<div className='grid lg:grid-cols-5 gap-4'>
				<div className='lg:col-span-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm'>
					<div className='flex items-center justify-between mb-5'>
						<div>
							<h3 className='font-black text-slate-900 dark:text-white text-sm'>{t('chart_monthly_revenue')}</h3>
							<p className='text-[10px] text-slate-400 mt-0.5'>{t('chart_year_label', { year: new Date().getFullYear() })}</p>
						</div>
				...
				<div className='lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm'>
					<div className='flex items-center justify-between mb-4'>
						<h3 className='font-black text-slate-900 dark:text-white text-sm'>{t('course_breakdown_title')}</h3>
							...
							<div key={c.name}>
								<div className='flex items-center justify-between mb-1.5'>
									...
									<div className='text-right'>
										...
										<span className='text-[10px] text-slate-400 ml-1'>{t('student_count_suffix')}</span>
									</div>
								</div>
							</div>
						)) : (
							<p className='text-xs text-slate-400 text-center py-4'>{t('course_breakdown_empty')}</p>
						)}
					</div>
					<div className='grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800'>
						<div className='bg-slate-50 dark:bg-slate-800 rounded-xl p-2.5'>
							<p className='text-[10px] text-slate-400 mb-0.5'>{t('stat_total_students')}</p>
							<p className='text-base font-black text-slate-900 dark:text-white'>{studentCount}</p>
						</div>
						<div className='bg-slate-50 dark:bg-slate-800 rounded-xl p-2.5'>
							<p className='text-[10px] text-slate-400 mb-0.5'>{t('stat_total_revenue')}</p>
			...
			<div className='bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden'>
				<div className='flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800'>
					<div>
						<h3 className='font-black text-slate-900 dark:text-white text-sm'>{t('recent_students_title')}</h3>
						<p className='text-[10px] text-slate-400 mt-0.5'>{t('recent_students_sub')}</p>
					</div>
				</div>
					...
					<table className='w-full text-sm'>
						<thead>
							<tr className='border-b border-slate-100 dark:border-slate-800'>
								<th className='text-left text-[10px] font-black text-slate-400 uppercase tracking-widest px-5 py-2.5'>{t('table_student')}</th>
								<th className='text-left text-[10px] font-black text-slate-400 uppercase tracking-widest px-3 py-2.5 hidden sm:table-cell'>{t('table_course')}</th>
								<th className='text-left text-[10px] font-black text-slate-400 uppercase tracking-widest px-3 py-2.5 hidden md:table-cell'>{t('table_date')}</th>
								<th className='text-left text-[10px] font-black text-slate-400 uppercase tracking-widest px-3 py-2.5'>{t('table_status')}</th>
								<th className='text-right text-[10px] font-black text-slate-400 uppercase tracking-widest px-5 py-2.5'>{t('table_payment')}</th>
							</tr>
						</thead>
						<tbody className='divide-y divide-slate-100 dark:divide-slate-800'>
							{recentStudents.length > 0 ? recentStudents.map((s, idx) => (
								<tr key={`${s.name}-${s.date}-${idx}`} className='hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group'>
									...
									<td className='px-3 py-3'><StatusBadge status={s.status} t={t} /></td>
									<td className='px-5 py-3 text-right'>
										<span className='text-xs font-black text-slate-900 dark:text-white'>{s.amount}</span>
									</td>
								</tr>
							)) : (
								<tr><td colSpan={5} className='text-center py-8 text-xs text-slate-400'>{t('recent_students_empty')}</td></tr>
							)}
						</tbody>
					</table>
				</div>
			...
			<div className='grid sm:grid-cols-3 gap-4'>
				{[
					{ label: t('stat_new_this_month'), value: String(newThisMonth), sub: t('stat_registered'), icon: Users, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-500/10' },
					{ label: t('stat_completed_courses'), value: String(certCount), sub: t('stat_certificated'), icon: GraduationCap, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-500/10' },
					{ label: t('stat_pending_payments'), value: String(pendingRevenue ? `$${pendingRevenue}` : '$0'), sub: t('stat_unpaid'), icon: CreditCard, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-500/10' },
				].map(item => {
