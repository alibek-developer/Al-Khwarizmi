...
// ─── EN → UZ avtomatik mapping ─────────────────────────────────────────────
const COURSE_TITLES = [
	{ en: 'Web Development', uz: 'Veb Dasturlash' },
	{ en: 'English Course', uz: 'Ingliz tili kursi' },
	{ en: 'Data Science', uz: "Ma'lumotlar fani" },
	{ en: 'AI & ML', uz: 'AI va ML' },
]
...
function CourseFormFields({
	...
	const [mentors, setMentors] = useState<
		{ id: number; full_name: string; specialty_en: string; image_url: string }[]
	>([])
	...
	const handleImg = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (!error) {
			...
			onChange({ ...form, image_url: data.publicUrl })
		}
	}

	// EN tanlanganda UZ avtomatik to'lsin
	const handleTitleChange = (enValue: string) => {
		const found = COURSE_TITLES.find(c => c.en === enValue)
		onChange({ ...form, title_en: enValue, title_uz: found?.uz ?? '' })
	}
	...
	return (
		<div className='space-y-4'>
			...
			{/* Kurs nomi — SELECT (EN tanlansa UZ avtomatik) */}
			<div>
				...
				{/* UZ nomi preview */}
				{form.title_uz && (
					<p className='mt-1.5 text-[11px] text-blue-600 dark:text-blue-400 font-semibold pl-1'>
						✓ O'zbekcha: {form.title_uz}
					</p>
				)}
			</div>
			...
			<div className='grid grid-cols-2 gap-3'>
				<div>
					...
					<input
						...
						onChange={e =>
							onChange({ ...form, lessons_count: +e.target.value })
						}
					/>
				</div>
				<div>
			...
			<div>
				...
				<select
					...
					onChange={e =>
						onChange({
							...form,
							mentor_id: e.target.value ? +e.target.value : null,
						})
					}
				>
					...
					<div className='mt-2 flex items-center gap-3 bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 rounded-xl px-3 py-2.5'>
						...
						<div className='flex-1 min-w-0'>
							...
							<p className='text-[10px] text-blue-500 dark:text-blue-400 truncate'>
								{selectedMentor.specialty_en} · Biriktirildi ✓
							</p>
						</div>
						<button
							type='button'
							onClick={() => onChange({ ...form, mentor_id: null })}
...
function EditCourseForm({
	...
	// EN tanlanganda UZ avtomatik
	const handleTitleChange = (enValue: string) => {
		const found = COURSE_TITLES.find(c => c.en === enValue)
		set({ title_en: enValue, title_uz: found?.uz ?? editItem.title_uz })
	}
	...
	return (
		<div className='space-y-4'>
			...
			<div className='grid grid-cols-2 gap-3'>
				<div>
					<label className={labelCls}>Darslar</label>
					<input
						className={inputCls}
						type='number'
						value={editItem.lessons_count}
