...
// ─── Default kurslar (statik) ──────────────────────────────────────────────
const DEFAULT_COURSES: CourseOption[] = [
  { id: 1, title_en: "Web Development", title_uz: "Veb Dasturlash" },
  { id: 2, title_en: "English Course", title_uz: "Ingliz tili" },
  { id: 3, title_en: "Data Science", title_uz: "Ma'lumotlar Fanlari" },
  { id: 4, title_en: "AI & ML", title_uz: "Sun'iy Intellekt" },
];
...
function CourseForm({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Kurs tanlash — courses jadvalidan */}
      <div>
        <label className={labelCls}>
          Kurs *
          {coursesLoading && (
        ...
        <select
          ...
          value={form.title_en}
          onChange={(e) => {
            const sel = coursesList.find((c) => c.title_en === e.target.value);
            set({
              title_en: sel?.title_en || "",
              title_uz: sel?.title_uz || "",
            });
          }}
        ...
        {form.title_en && (
          <p className="mt-1 text-[11px] text-blue-600 dark:text-blue-400 font-semibold flex items-center gap-1">
            <CheckCircle2 className="w-3 h-3" />
            {form.title_en}
            {form.title_uz ? ` · ${form.title_uz}` : ""}
          </p>
        )}
      </div>
...
export default function AdminCoursesPage() {
  ...
  const [coursesList] = useState<CourseOption[]>(DEFAULT_COURSES);
